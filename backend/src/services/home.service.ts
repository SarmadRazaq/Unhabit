...
export async function getDashboard(userId: string) {
  ...
  let currentDayNumber = 1; // Default to day 1 (journey_days are 1-indexed)
  let journeyProgress = 0;
  let habitHealth = 0;

  if (activeJourney) {
    if (activeJourney.start_date) {
      const startDate = new Date(activeJourney.start_date);
      startDate.setHours(0, 0, 0, 0);
      const diffTime = today.getTime() - startDate.getTime();
      // If journey hasn't started yet (future start_date), default to day 1
      if (diffTime >= 0) {
