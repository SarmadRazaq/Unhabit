...
import { API_BASE_URL, apiRequest, printSummary, testResults, recordTest, testContext, prisma, verifyOperation, verifyByGet } from './test-helpers';
...
async function runAllTests() {
  try {
    ...
    // Pre-flight: ensure tests are targeting the backend API (not another local app)
    const preflightRes = await apiRequest('GET', '/health');
    const preflightOk = preflightRes.status === 200 && preflightRes.data?.ok === true;
    if (!preflightOk) {
      const isHtmlResponse = typeof preflightRes.data === 'string' && preflightRes.data.includes('<!DOCTYPE html>');
      const responsePreview = typeof preflightRes.data === 'string'
        ? preflightRes.data.slice(0, 160)
        : JSON.stringify(preflightRes.data).slice(0, 160);

      throw new Error(
        isHtmlResponse
          ? `Pre-flight failed: ${API_BASE_URL}/health returned HTML (likely wrong local service). Set API_BASE_URL to your backend, e.g. http://localhost:3000/api`
          : `Pre-flight failed: expected 200 with { ok: true } from ${API_BASE_URL}/health, got ${preflightRes.status} with ${responsePreview}`
      );
    }
