...
export async function testNewEndpoints(userToken: string, userId: string) {
  ...
  recordTest('New 1.10 Mark Notification as Read - Invalid ID format', invalidReadRes.status === 400);
  ...
  recordTest('New 1.15 Delete Notification - Invalid ID format', invalidDeleteRes.status === 400);
  ...
  if (challengeId) {
    ...
    if (completeValid) {
      ...
      recordTest('New 2.9.1 Complete Challenge - XP Awarded', 
        xpEarned !== undefined && xpEarned > 0 && pointsRes.status === 200,
        xpEarned === undefined
          ? 'XP not returned in response'
          : (pointsRes.status !== 200 ? `Expected /rewards/points 200, got ${pointsRes.status}` : undefined));
    }
  ...
  recordTest('New 7.10 Get Buddy Profile - Invalid ID format', invalidProfileRes.status === 400);
