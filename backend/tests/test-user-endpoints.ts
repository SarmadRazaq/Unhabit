...
export async function testUserEndpoints(userToken: string, userId: string) {
  ...
  recordTest('User 2.6 Complete Task - Invalid ID', invalidTaskRes.status === 400 || invalidTaskRes.status === 404,
    invalidTaskRes.status !== 400 && invalidTaskRes.status !== 404 ? `Expected 400/404, got ${invalidTaskRes.status}` : undefined);
  ...
  recordTest('User 3.5 Use Freeze', useFreezeRes.status === 200 || useFreezeRes.status === 400,
    useFreezeRes.status !== 200 && useFreezeRes.status !== 400 ? `Expected 200/400, got ${useFreezeRes.status}` : undefined);
  ...
  recordTest('User 3.6 Purchase Freeze', purchaseFreezeRes.status === 200 || purchaseFreezeRes.status === 400,
    purchaseFreezeRes.status !== 200 && purchaseFreezeRes.status !== 400 ? `Expected 200/400, got ${purchaseFreezeRes.status}` : undefined);
  ...
  recordTest('User 6.3 Start Focus - Invalid duration', invalidDurationRes.status === 400,
    invalidDurationRes.status !== 400 ? `Expected 400, got ${invalidDurationRes.status}` : undefined);
  ...
  recordTest('User 7.2 Continue with Penalty', penaltyRes.status === 200,
    penaltyRes.status !== 200 ? `Expected 200, got ${penaltyRes.status}` : undefined);
  ...
  recordTest('User 7.3 Use Protection', protectionRes.status === 200 || protectionRes.status === 400,
    protectionRes.status !== 200 && protectionRes.status !== 400 ? `Expected 200/400, got ${protectionRes.status}` : undefined);
  ...
  recordTest('User 7.4 Restart Plan', restartRes.status === 200 || restartRes.status === 400,
    restartRes.status !== 200 && restartRes.status !== 400 ? `Expected 200/400, got ${restartRes.status}` : undefined);
  ...
  recordTest('User 8.4 Generate Achievement Share - Invalid type', invalidTypeRes.status === 400,
    invalidTypeRes.status !== 400 ? `Expected 400, got ${invalidTypeRes.status}` : undefined);
